...
class RegistrationController extends AbstractController
{
        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->isValid()) {
                ...
                $this->emailVerifier->sendEmailConfirmation('app_verify_email', $user,
                    (new TemplatedEmail())
                        ->from(new Address($_ENV['MAIL_ADDRESS'], 'Stubborn Company'))
    ...
    public function verifyUserEmail(Request $request, TranslatorInterface $translator, CustomerRepository $customerRepository): Response
    {
        ...
        $this->addFlash('success', 'Merci, votre adresse mail a bien été validée. Votre compte est maintenant activé.');
        return $this->redirectToRoute('app_home');
    }
}
